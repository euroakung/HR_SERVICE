...
import { catchError, debounceTime, distinctUntilChanged, switchMap, tap, filter, map, distinct, toArray } from 'rxjs/operators';
import { concat, Observable, of, Subject, combineLatest } from 'rxjs';
...
public loading = false;
  ...
  selectedleader: Observable<any[]>;
   selecttest: Observable<any[]>;
  selecteddirector: Observable<any[]>;ApiInformService: any;
;
  selectedInform:Observable<any[]>;
  states: Observable<any[]>;;
  filteredStates$: Observable<Approver[]>;
 ...
 this.selectedInform = this.ApiInformService.getInform();
  ...
  this.selectedCountry = this.apiCountry.getCountry() ;
    ...
    this.leaveService.getLeaveItemsById(params.get('id')).subscribe(res => {
      ...
      this.registerForm.get('leader').setValue('');
      this.registerForm.get('director').setValue('');
      this.registerForm.get('approve').setValue('');
